            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {paginatedItems.map((item, index) => (
                <AnimatedSection key={item.id} delay={index * 0.05}>
                  <Link
                    to={`/riwayat/${item.id}`}
                    className="border border-black rounded-lg hover:border-gray-600 shadow-sm hover:shadow-md hover:scale-[1.02] transition-transform duration-200 p-4 block h-full"
                  >
